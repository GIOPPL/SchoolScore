...
class DownScoreMolder() : ReportServerModel {
    ...
    fun getScore(account: String, psw: String) {
    ...
    //xml文件处理成类
    private fun XmlManage(xml: Document) {
    ...
    fun getDocument(): Document? {
        ...
        when (cookies) {
            "100" -> {
                mList!!.add(ScoreEntity("未获取到SESSION")); return null
            }
            "101" -> {
                mList!!.add(ScoreEntity("学号不存在"));return null
            }
            "102" -> {
                mList!!.add(ScoreEntity("密码不正确"));return null
            }
            "103" -> {
                mList!!.add(ScoreEntity("网页格式错误")); return null
            }
        }
        try {
